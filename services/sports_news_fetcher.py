...
def fetch_espn_sports_articles(max_articles=4):
    ...
    for feed_url in ESPN_FEEDS:
        try:
            ...
            articles = fetch_espn_rss(feed_url, max_articles=max_articles)

            if articles:
                print(f"Loaded ESPN sports stories: {len(articles)}")
                return articles
    ...
    print(f"All ESPN sports feeds failed: {' | '.join(errors)}")
    return fallback_articles()


def fetch_espn_rss(feed_url, max_articles=4):
...
def fallback_articles():
    return [
        ...
        SportsArticle("Check your internet connection or ESPN RSS availability.", "", "ESPN"),
        SportsArticle("Sports news panel will keep using fallback stories.", "", "ESPN"),
