...
def moon_phase_fraction(moment=None):
    """Return lunar cycle progress: 0.0=new, 0.25=first quarter,
    ...
    elapsed_days = (
        moment_utc - KNOWN_NEW_MOON_UTC
    ).total_seconds() / 86400.0

    return (elapsed_days % SYNODIC_MONTH_DAYS) / SYNODIC_MONTH_DAYS


def moon_illumination_fraction(moment=None):
    """Return illuminated fraction from 0.0 to 1.0."""
...
class WeatherRow(QWidget):
    ...
    def apply_text_colors(self):
        dark_weather_art = self.is_night or self.condition in {"rain", "storm"}
    ...
    def update_weather(
        ...
        self.condition = condition
        self.is_night = is_night
        self.moon_datetime = moon_datetime
        ...
        dark_weather_art = is_night or condition in {"rain", "storm"}
...
class WeatherPanel(QWidget):
    hour_selected = Signal(object, int)
    ...
    def __init__(self):
        ...
        for index, (temp, icon, hour, condition, is_night) in enumerate(rows):
            ...
            row_widget.clicked.connect(
                lambda selected_index=index: self.emit_selected_hour(selected_index)
            )
            self.row_widgets.append(row_widget)
            main.addWidget(row_widget, 1)

    def _next_full_hour_rows(self):
        parsed_rows = []

        for original_index, row in enumerate(self.current_rows):
            raw_start = str(
    ...
    def emit_selected_hour(self, index):
        if 0 <= index < len(self.visible_rows):
            original_index, row = self.visible_rows[index]
            self.hour_selected.emit(row, original_index)

    def update_weather_rows(self, rows):
        self.current_rows = list(rows or [])
        self.visible_rows = self._next_full_hour_rows()

        weather_row_widgets = list(self.row_widgets or [])

        if not weather_row_widgets:
            print("Weather panel update skipped: no weather row widgets found.")
            return

        for index, weather_row_widget in enumerate(weather_row_widgets):
            if index >= len(self.visible_rows):
                weather_row_widget.hide()
                continue

            _original_index, row = self.visible_rows[index]
        ...
        first_label = (
            self.visible_rows[0][1].time_label
