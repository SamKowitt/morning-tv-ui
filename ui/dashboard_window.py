...
from services.news_fetcher import (
    ...
    get_news_source_display,
)
from services.sports_games_fetcher import (
    fetch_current_sports_games,
    fetch_mlb_boxscore,
    fetch_espn_boxscore,
)
...
from ui.panels.weather_radar_dialog import WeatherRadarDialog
...
class MarketTapeReplacementCheckbox(QCheckBox):
    def paintEvent(self, event):
        ...
        pen = QPen(QColor("#f7f0df"))
        pen.setWidth(2)
        painter.setPen(pen)

        x = 5
        y = max(3, int((self.height() - 16) / 2) + 3)
...
class WeatherRadarPreloadWorker(QObject):
    ...
    def __init__(
        ...
        self.default_zoom = int(default_zoom)
        self.background_zooms = list(background_zooms or [])

    def run(self):
        try:
            report = preload_weathercom_radar_loops(
            ...
            self.finished.emit(
                int(report.get("ready", 0)),
                int(report.get("failed", 0)),
                int(report.get("requested", 0)),
            )
...
class MoreNewsFetchWorker(QObject):
    finished = Signal(int, str, object)
    failed = Signal(int, str)

    def __init__(self, request_id, source_key):
        super().__init__()
        self.request_id = request_id
    ...
    def run(self):
        try:
            source_key, source_name, ranked_candidates = fetch_ranked_candidates(
                self.source_key
            )
...
class SportsBoxscorePrefetchWorker(QObject):
    finished = Signal(int)
    failed = Signal(str)
    ...
    def run(self):
        ...
        try:
            for league_data in self.leagues:
                league = str(
                    getattr(league_data, "league", "") or ""
                ).upper()

                for game in getattr(league_data, "games", []) or []:
                    if not bool(getattr(game, "is_clickable", False)):
                        continue
                    ...
                    elif league in {"NFL", "NBA"}:
                        fetch_espn_boxscore(league, game_id)
                    else:
                        continue

                    warmed += 1
                    print(f"Box-score preload ready: {league} {game_id}")
...
class DashboardWindow(QMainWindow):
    def __init__(self):
        ...
        self.recurring_calendar_events = self.load_recurring_calendar_events()
        self.recurring_event_rows = {}
        self.recurring_draft_event_id = None
        self.recurring_add_button = None

        self.news_request_id = 0
        self.more_news_request_id = 0
        self.market_news_request_id = 0

        self.more_news_thread = None
        self.more_news_worker = None
        ...
        self.sports_games_has_loaded = False
        ...
        self.latest_sports_leagues = []
        self.radar_preload_waiting_for_boxscores = False

        self.sports_games_refresh_timer = QTimer(self)
        self.sports_games_refresh_timer.setInterval(5 * 60 * 1000)
        self.sports_games_refresh_timer.timeout.connect(
            self.start_sports_games_fetch
        )

        self.mini_news_refresh_timer = QTimer(self)
        self.mini_news_refresh_timer.setInterval(5 * 60 * 1000)
        self.mini_news_refresh_timer.timeout.connect(
        ...
        self.weather_radar_refresh_timer = QTimer(self)
        self.weather_radar_refresh_timer.setInterval(5 * 60 * 1000)
        self.weather_radar_refresh_timer.timeout.connect(
        ...
        # Warm both hidden mini-news panels and their article text immediately.
        self.start_mini_news_background_refresh()
        self.mini_news_refresh_timer.start()

        self.start_sports_news_fetch()
        self.start_sports_games_fetch()
        self.sports_games_refresh_timer.start()
        ...
        if self.hide_reminders:
            self.start_more_news_fetch()
    ...
    def load_saved_market_news_setting(self):
        raw_value = self.saved_settings.value(
            "replace_market_tape_with_news",
            False,
        )
    ...
    def load_saved_hide_reminders_setting(self):
        raw_value = self.saved_settings.value("hide_reminders", False)
    ...
    def save_news_source_settings(self):
        ...
        self.saved_settings.setValue("hide_reminders", self.hide_reminders)
    ...
    def eventFilter(self, watched, event):
        ...
        if (
            ...
            self.open_current_weather_radar_popup()
            return True
    ...
    def build_bottom_row(self):
        ...
        self.stocks_panel.hide_requested.connect(
            self.hide_market_tape_for_current_session
        )

        self.reminders = RemindersPanel()
        self.reminders.hide_requested.connect(
            self.hide_reminders_for_current_session
        )

        self.more_news_card = NewsCard(
            headline="Loading more headlines...",
            source=get_news_source_display(self.left_news_source_key),
            image_label="MORE HEADLINES",
            variant="fox",
        )
        self.more_news_card.set_reminders_action_enabled(True)
        ...
        self.market_news_card.set_market_tape_action_enabled(True)
        self.market_news_card.market_tape_requested.connect(
            self.show_market_tape_for_current_session
        )

        self.bottom_left_container = QWidget()
        self.bottom_left_container.setMinimumWidth(0)
        self.bottom_left_container.setSizePolicy(
            QSizePolicy.Ignored,
            QSizePolicy.Expanding,
        )

        self.bottom_left_stack = QStackedLayout()
        self.bottom_left_stack.setContentsMargins(0, 0, 0, 0)
        self.bottom_left_container.setLayout(self.bottom_left_stack)

        self.bottom_left_stack.addWidget(self.reminders)
        self.bottom_left_stack.addWidget(self.more_news_card)
        self.bottom_left_stack.setCurrentWidget(self.reminders)

        self.bottom_right_container = QWidget()
        self.bottom_right_container.setMinimumWidth(0)
        self.bottom_right_container.setSizePolicy(
            QSizePolicy.Ignored,
            QSizePolicy.Expanding,
        )

        self.bottom_right_stack = QStackedLayout()
        self.bottom_right_stack.setContentsMargins(0, 0, 0, 0)
        self.bottom_right_container.setLayout(self.bottom_right_stack)

        self.bottom_right_stack.addWidget(self.stocks_panel)
        self.bottom_right_stack.addWidget(self.market_news_card)

        if self.replace_market_tape_with_news:
            self.bottom_right_stack.setCurrentWidget(self.market_news_card)
        else:
            self.bottom_right_stack.setCurrentWidget(self.stocks_panel)

        for widget in [
            self.reminders,
            self.more_news_card,
            self.stocks_panel,
            self.market_news_card,
        ]:
        ...
        layout.addWidget(self.bottom_left_container, 1)
        layout.addWidget(self.bottom_right_container, 1)
    ...
    def load_recurring_calendar_events(self):
        raw_value = str(
            self.saved_settings.value("recurring_calendar_events", "[]") or "[]"
        ).strip()
        ...
        for item in items:
            ...
            if frequency not in {"weekly", "monthly", "annually", "specific"}:
                continue
    ...
    def save_recurring_calendar_events(self):
        self.saved_settings.setValue(
            "recurring_calendar_events",
            json.dumps(self.recurring_calendar_events),
        )
        self.saved_settings.sync()

        print(
            "Saved recurring calendar events: "
            f"{len(self.recurring_calendar_events)}"
        )

    def recurring_month_choices(self):
        return [
            ("January", 1),
            ("February", 2),
            ("March", 3),
    ...
    def recurring_valid_days_for_month(self, month_number):
        try:
        ...
        # Leap-year baseline includes February 29 but prevents invalid dates
        # such as February 30 and April 31.
        return list(range(1, calendar_module.monthrange(2024, month_number)[1] + 1))

    def build_recurring_combo(self, placeholder, choices):
        combo = QComboBox()
    ...
    def build_recurring_action_button(self, text, tooltip, positive):
        ...
        if positive:
            ...
            button.setStyleSheet("""
                QPushButton {
                    background: rgba(173, 41, 35, 0.14);
                    color: #9f211b;
                    border: 1px solid rgba(159, 33, 27, 0.68);
                    border-radius: 6px;
                    font-size: 14px;
                    font-weight: 1000;
                    padding: 0px;
                ...
                QPushButton:hover {
                    background: rgba(173, 41, 35, 0.26);
                }
            """)

        return button

    def clear_recurring_detail_layout(self, row):
    ...
    def populate_recurring_detail_fields(self, row, restore_data=None):
        self.clear_recurring_detail_layout(row)

        frequency = row["frequency_combo"].currentData()
        restore_data = restore_data or {}

        if frequency == "weekly":
            day_combo = self.build_recurring_combo(
                "Day",
                [
                    ("Sunday", "Sunday"),
                    ("Monday", "Monday"),
            ...
            saved_day = restore_data.get("day", "")
            index = day_combo.findData(saved_day)

            if index >= 0:
                day_combo.setCurrentIndex(index)

            day_combo.currentIndexChanged.connect(
                lambda _index, current_row=row:
                    self.try_save_recurring_event(current_row)
            )

            row["detail_layout"].addWidget(day_combo)
            row["detail_widgets"] = [day_combo]

        elif frequency == "monthly":
            choices = [(str(day), day) for day in range(1, 32)]
            choices.append(("Last day of month", "last_day"))

            day_combo = self.build_recurring_combo("Day", choices)
            ...
            month_combo = self.build_recurring_combo(
                "Month",
                self.recurring_month_choices(),
            )
            day_combo = self.build_recurring_combo("Day", [])

            saved_month = restore_data.get("month", "")
            month_index = month_combo.findData(saved_month)

            if month_index >= 0:
                month_combo.setCurrentIndex(month_index)

            def refresh_day_choices():
                selected_day = day_combo.currentData()

                if not selected_day:
                    selected_day = restore_data.get("day", "")

                while day_combo.count() > 1:
                    day_combo.removeItem(1)

                for day in self.recurring_valid_days_for_month(
                    month_combo.currentData()
                ):
                    day_combo.addItem(str(day), day)

                saved_index = day_combo.findData(selected_day)

                if saved_index >= 0:
                    day_combo.setCurrentIndex(saved_index)
                else:
                    day_combo.setCurrentIndex(0)

            month_combo.currentIndexChanged.connect(
                lambda _index: refresh_day_choices()
            )
            month_combo.currentIndexChanged.connect(
                lambda _index, current_row=row:
                    self.try_save_recurring_event(current_row)
            ...
            row["detail_layout"].addWidget(month_combo)
            row["detail_layout"].addWidget(day_combo)
            row["detail_widgets"] = [month_combo, day_combo]

    def recurring_record_from_row(self, row):
        title = row["title_input"].text().strip()
        frequency = row["frequency_combo"].currentData()

        if not title or frequency not in {"weekly", "monthly", "annually"}:
            return None
        ...
        details = row["detail_widgets"]

        if frequency in {"weekly", "monthly"}:
            if len(details) != 1:
                return None

            day = details[0].currentData()
            ...
            month = details[0].currentData()
            day = details[1].currentData()

            if month in {"", None} or day in {"", None}:
                return None

            valid_days = self.recurring_valid_days_for_month(month)
    ...
    def try_save_recurring_event(self, row):
        record = self.recurring_record_from_row(row)
        ...
        for index, existing in enumerate(self.recurring_calendar_events):
            if existing.get("id") == event_id:
                self.recurring_calendar_events[index] = record
                replaced = True
                break
        ...
        if not row["saved"]:
            row["saved"] = True
            self.recurring_draft_event_id = None
            self.add_recurring_row_controls(row)
            self.refresh_recurring_add_button()

    def add_recurring_row_controls(self, row):
        if row.get("controls_added", False):
            return
        ...
        plus_button = self.build_recurring_action_button(
            "+",
            "Add recurring event",
            positive=True,
        )
        plus_button.clicked.connect(self.start_new_recurring_event)

        delete_button = self.build_recurring_action_button(
            "🗑",
            "Delete recurring event",
            positive=False,
        )
        delete_button.clicked.connect(
            lambda _checked=False, event_id=row["event_id"]:
                self.delete_recurring_event(event_id)
        )

        controls.addWidget(plus_button, 0, Qt.AlignLeft)
        controls.addStretch(1)
        controls.addWidget(delete_button, 0, Qt.AlignRight)

        row["layout"].addLayout(controls)
        row["controls_added"] = True

    def on_recurring_frequency_changed(self, row):
        self.populate_recurring_detail_fields(row)
        self.try_save_recurring_event(row)

    def add_recurring_event_editor(self, event=None):
        event = dict(event or {})
        event_id = str(event.get("id") or uuid.uuid4().hex)

        frame = QFrame()
        frame.setObjectName("RecurringCalendarEventRow")
        frame.setStyleSheet("""
            QFrame#RecurringCalendarEventRow {
        ...
        title_input.setText(str(event.get("title", "") or ""))
        title_input.setMinimumWidth(170)

        frequency_combo = self.build_recurring_combo(
            "Frequency",
            [
                ("Weekly", "weekly"),
                ("Monthly", "monthly"),
        ...
        saved_frequency = event.get("frequency", "")
        frequency_index = frequency_combo.findData(saved_frequency)
        ...
        top_row.addWidget(title_input, 2)
        top_row.addWidget(frequency_combo, 1)
        top_row.addLayout(detail_layout, 1)
        ...
        row = {
            ...
            "frequency_combo": frequency_combo,
        ...
        self.recurring_event_rows[event_id] = row

        title_input.textChanged.connect(
            lambda _text, current_row=row:
                self.try_save_recurring_event(current_row)
        )
        frequency_combo.currentIndexChanged.connect(
            lambda _index, current_row=row:
                self.on_recurring_frequency_changed(current_row)
        )

        self.recurring_events_layout.addWidget(frame)

        if saved_frequency:
            self.populate_recurring_detail_fields(row, event)

        if event:
            self.add_recurring_row_controls(row)
        else:
            self.recurring_draft_event_id = event_id
            self.refresh_recurring_add_button()
            title_input.setFocus()

    def start_new_recurring_event(self):
        if self.recurring_draft_event_id:
            row = self.recurring_event_rows.get(self.recurring_draft_event_id)
        ...
        self.add_recurring_event_editor()

    def delete_recurring_event(self, event_id):
        self.recurring_calendar_events = [
            event for event in self.recurring_calendar_events
            if event.get("id") != event_id
        ]
        self.save_recurring_calendar_events()

        row = self.recurring_event_rows.pop(event_id, None)

        if row:
            self.recurring_events_layout.removeWidget(row["widget"])
            row["widget"].deleteLater()

        if self.recurring_draft_event_id == event_id:
            self.recurring_draft_event_id = None

        self.refresh_recurring_add_button()

    def refresh_recurring_add_button(self):
        if self.recurring_add_button is None:
            return

        has_saved_events = bool(self.recurring_calendar_events)
        has_draft = bool(self.recurring_draft_event_id)

        self.recurring_add_button.setVisible(
            not has_saved_events and not has_draft
        )

    def build_recurring_calendar_section(self, parent_layout):
        button = QPushButton("Add Calendar Event")
        button.setCursor(Qt.PointingHandCursor)
        ...
        """)
        button.clicked.connect(self.open_recurring_events_popup)

        parent_layout.addSpacing(6)
        parent_layout.addWidget(button)

    def clear_specific_date_button_value(self, button):
        button.setProperty("selected_date_iso", "")
        button.setText("")

    def set_specific_date_button_value(self, button, date_value):
        if not isinstance(date_value, QDate) or not date_value.isValid():
            self.clear_specific_date_button_value(button)
            return
        ...
        button.setProperty(
            "selected_date_iso",
            date_value.toString(Qt.ISODate),
        )
        button.setText(date_value.toString("MMMM d, yyyy"))

    def open_specific_date_picker(self):
        popup = getattr(self, "_recurring_events_popup", None)

        if not popup:
            return

        parent_dialog = popup["dialog"]
        date_button = popup["specific_date_button"]

        saved_value = str(
            date_button.property("selected_date_iso") or ""
        ).strip()

        selected_date = QDate.fromString(saved_value, Qt.ISODate)

        if not selected_date.isValid() or selected_date < QDate.currentDate():
            selected_date = QDate.currentDate()

        picker_dialog = QDialog(parent_dialog)
        picker_dialog.setWindowTitle("Choose Specific Date")
        picker_dialog.setModal(True)
        picker_dialog.setStyleSheet("""
            QDialog {
            ...
            QCalendarWidget QWidget {
                alternate-background-color: #f7f0df;
            }

            QCalendarWidget QToolButton {
                color: #2d2114;
                background: #d8c7a4;
                border: 1px solid rgba(83, 59, 33, 0.42);
                border-radius: 5px;
                font-weight: 900;
                padding: 4px;
            }
            ...
            QCalendarWidget QAbstractItemView:enabled {
                color: #2d2114;
                background: #fffaf0;
                selection-background-color: #8a5a24;
        ...
        picker_dialog.setLayout(picker_layout)
        ...
        def apply_selected_date():
            self.set_specific_date_button_value(
                date_button,
                calendar.selectedDate(),
            )
            picker_dialog.accept()

        select_button.clicked.connect(apply_selected_date)
    ...
    def open_recurring_events_popup(self):
        ...
        dialog.setStyleSheet("""
            ...
            QLabel#RecurringDialogHint,
            QLabel#CalendarChoiceLabel,
            QLabel#CalendarOrLabel {
                color: rgba(45, 33, 20, 0.76);
                background: transparent;
                font-size: 11px;
                font-weight: 900;
            }
            ...
            QTableWidget {
                ...
                border: 1px solid rgba(83, 59, 33, 0.32);
                border-radius: 7px;
                gridline-color: rgba(83, 59, 33, 0.18);
                font-size: 11px;
                font-weight: 700;
            }
            ...
            QHeaderView::section {
                ...
                color: #2d2114;
                border: none;
                border-bottom: 1px solid rgba(83, 59, 33, 0.38);
                padding: 5px;
                font-size: 10px;
                font-weight: 1000;
            }
        ...
        title = QLabel("ADD CALENDAR EVENT")
        title.setObjectName("RecurringDialogTitle")
        title.setAlignment(Qt.AlignCenter)

        hint = QLabel(
            "Choose one schedule type below. Saved events do not appear in Upcoming."
        )
        hint.setObjectName("RecurringDialogHint")
        ...
        title_input.setObjectName("SettingsLineEdit")
        title_input.setPlaceholderText("e.g., Trash Day")
        title_input.setMinimumWidth(240)
        root.addWidget(title_input)

        specific_row = QHBoxLayout()
        specific_row.setContentsMargins(0, 0, 0, 0)
        specific_row.setSpacing(8)

        specific_label = QLabel("Specific Date:")
        specific_label.setObjectName("CalendarChoiceLabel")

        specific_date_button = QPushButton()
        specific_date_button.setCursor(Qt.PointingHandCursor)
        specific_date_button.setMinimumWidth(190)
        specific_date_button.setToolTip("Choose today or a future date")
        specific_date_button.setStyleSheet("""
            QPushButton {
                background: #fff7e7;
                color: #2d2114;
                border: 1px solid rgba(166, 110, 38, 0.65);
                border-radius: 10px;
                padding: 8px 14px;
                font-size: 14px;
                font-weight: 900;
            ...
            QPushButton:hover {
                background: #fff0d4;
                border-color: rgba(142, 90, 28, 0.90);
            }
        """)
        self.clear_specific_date_button_value(specific_date_button)

        specific_row.addWidget(specific_label)
        specific_row.addWidget(specific_date_button)
        specific_row.addStretch(1)

        root.addLayout(specific_row)

        or_label = QLabel("OR")
        or_label.setObjectName("CalendarOrLabel")
        or_label.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        root.addWidget(or_label)

        recurring_row = QHBoxLayout()
        recurring_row.setContentsMargins(0, 0, 0, 0)
        recurring_row.setSpacing(8)

        recurring_label = QLabel("Reoccuring Event:")
        recurring_label.setObjectName("CalendarChoiceLabel")
        ...
        detail_holder.setLayout(detail_layout)

        recurring_row.addWidget(recurring_label)
        recurring_row.addWidget(frequency_combo)
        recurring_row.addWidget(detail_holder, 1)
        ...
        table.setHorizontalHeaderLabels(
            ["Event", "Frequency", "Schedule", "Delete"]
        )
        ...
        popup = {
            "dialog": dialog,
            "title_input": title_input,
            "specific_date_button": specific_date_button,
            "frequency_combo": frequency_combo,
            "detail_layout": detail_layout,
            "detail_widgets": [],
            "table": table,
        ...
        self._recurring_events_popup = popup

        specific_date_button.clicked.connect(self.open_specific_date_picker)

        frequency_combo.currentIndexChanged.connect(
            lambda _index: self.refresh_recurring_popup_detail_fields()
        )
        add_button.clicked.connect(self.add_recurring_popup_event)
        close_button.clicked.connect(dialog.accept)

        self.refresh_recurring_popup_detail_fields()
        self.refresh_recurring_popup_table()

        dialog.exec()
        self._recurring_events_popup = None

    def refresh_recurring_popup_detail_fields(self):
        popup = getattr(self, "_recurring_events_popup", None)

        if not popup:
            return

        layout = popup["detail_layout"]
        ...
        popup["detail_widgets"] = []

        frequency = popup["frequency_combo"].currentData()

        if frequency == "weekly":
            combo = self.build_recurring_combo(
                "Day",
                [
                    ("Sunday", "Sunday"),
                    ("Monday", "Monday"),
            ...
            popup["detail_widgets"] = [combo]

        elif frequency == "monthly":
            choices = [(str(day), day) for day in range(1, 32)]
            choices.append(("Last day of month", "last_day"))

            combo = self.build_recurring_combo("Day", choices)
            layout.addWidget(combo)
            popup["detail_widgets"] = [combo]
            ...
            def refresh_days():
                ...
                index = day_combo.findData(selected_day)
            ...
            month_combo.currentIndexChanged.connect(
                lambda _index: refresh_days()
            )
            refresh_days()

            layout.addWidget(month_combo)
            layout.addWidget(day_combo)
            popup["detail_widgets"] = [month_combo, day_combo]

    def add_recurring_popup_event(self):
        popup = getattr(self, "_recurring_events_popup", None)
        ...
        frequency = popup["frequency_combo"].currentData()
        detail_widgets = popup["detail_widgets"]
        specific_date_text = str(
            popup["specific_date_button"].property("selected_date_iso") or ""
        ).strip()
        specific_date = QDate.fromString(
            specific_date_text,
            Qt.ISODate,
        )
        ...
        if not title:
            QMessageBox.warning(
                popup["dialog"],
                "Missing Event Name",
                "Enter an event name before adding it.",
            )
            popup["title_input"].setFocus()
            return
        ...
        if frequency in {"weekly", "monthly", "annually"}:
            record["frequency"] = frequency

            if frequency in {"weekly", "monthly"}:
                if len(detail_widgets) != 1:
                    return

                day = detail_widgets[0].currentData()

                if day in {"", None}:
                    QMessageBox.warning(
                        popup["dialog"],
                        "Missing Schedule",
                        "Select the recurring event day.",
                    )
                    detail_widgets[0].setFocus()
                    return

                record["day"] = day

            elif frequency == "annually":
                if len(detail_widgets) != 2:
                    return

                month = detail_widgets[0].currentData()
                day = detail_widgets[1].currentData()

                if month in {"", None} or day in {"", None}:
                    QMessageBox.warning(
                        popup["dialog"],
                        "Missing Schedule",
                        "Select both an annual month and day.",
                    )
                    return

                if int(day) not in self.recurring_valid_days_for_month(month):
                    QMessageBox.warning(
                        popup["dialog"],
                        "Invalid Date",
                        "That annual date is not valid.",
                    )
                    return
            ...
            if not specific_date.isValid():
                QMessageBox.warning(
                    popup["dialog"],
                    "Missing Date",
                    "Choose a specific date or select a recurring event schedule.",
                )
                return

            if specific_date < QDate.currentDate():
                QMessageBox.warning(
                    popup["dialog"],
                    "Past Date",
                    "Specific dates must be today or later.",
                )
                return

            record["frequency"] = "specific"
            record["date"] = specific_date.toString("yyyy-MM-dd")
        ...
        self.clear_specific_date_button_value(
            popup["specific_date_button"],
        )
        popup["frequency_combo"].setCurrentIndex(0)

        self.refresh_recurring_popup_detail_fields()
        self.refresh_recurring_popup_table()

    def recurring_popup_schedule_text(self, event):
        frequency = str(event.get("frequency", "") or "")

        if frequency == "weekly":
            return str(event.get("day", "") or "")
        ...
        if frequency == "annually":
            ...
            day = event.get("day", "")
            month_names = dict(self.recurring_month_choices())

            return f"{month_names.get(month, '')} {day}".strip()
    ...
    def refresh_recurring_popup_table(self):
        popup = getattr(self, "_recurring_events_popup", None)
        ...
        table = popup["table"]
        table.clearContents()
        table.setRowCount(len(self.recurring_calendar_events))
        popup["delete_buttons"] = []

        for row_index, event in enumerate(self.recurring_calendar_events):
            event_id = str(event.get("id", "") or "")
            title = str(event.get("title", "") or "")
            frequency = str(event.get("frequency", "") or "").title()
            schedule = self.recurring_popup_schedule_text(event)

            table.setItem(row_index, 0, QTableWidgetItem(title))
            table.setItem(row_index, 1, QTableWidgetItem(frequency))
            table.setItem(row_index, 2, QTableWidgetItem(schedule))

            delete_button = QPushButton("Delete")
            delete_button.setCursor(Qt.PointingHandCursor)
            delete_button.setToolTip(f"Delete {title}")
            delete_button.setFixedHeight(26)
            delete_button.setProperty("calendar_event_id", event_id)
            ...
            delete_button.clicked.connect(
                lambda _checked=False, button=delete_button:
                    self.delete_recurring_popup_event(
                        button.property("calendar_event_id")
                    )
            )

            popup["delete_buttons"].append(delete_button)
            table.setCellWidget(row_index, 3, delete_button)

    def delete_recurring_popup_event(self, event_id):
    ...
    def build_settings_page(self):
        ...
        self.hide_reminders_checkbox = MarketTapeReplacementCheckbox(
            "Always Hide Reminders"
        )
        self.hide_reminders_checkbox.setObjectName("HideRemindersCheckbox")
        self.hide_reminders_checkbox.setChecked(self.hide_reminders)
        self.hide_reminders_checkbox.setCursor(Qt.PointingHandCursor)
        self.hide_reminders_checkbox.setStyleSheet("""
            QCheckBox#HideRemindersCheckbox {
                color: #2d2114;
                background: transparent;
                font-size: 12px;
                font-weight: 800;
            ...
            QCheckBox#HideRemindersCheckbox::indicator {
                width: 16px;
                height: 16px;
            }

            QCheckBox#HideRemindersCheckbox::indicator:unchecked {
                background: rgba(255, 248, 236, 0.78);
                border: 1px solid rgba(83, 59, 33, 0.50);
                border-radius: 3px;
            }

            QCheckBox#HideRemindersCheckbox::indicator:checked {
                background: #72542d;
        ...
        """)
        left_settings_col.addWidget(self.hide_reminders_checkbox)

        left_settings_col.addWidget(right_label)
        left_settings_col.addWidget(self.right_news_combo)

        self.replace_market_tape_checkbox = MarketTapeReplacementCheckbox(
            "Always Hide Market Tape"
        )
        self.replace_market_tape_checkbox.setObjectName(
            "ReplaceMarketTapeCheckbox"
        )
        self.replace_market_tape_checkbox.setChecked(
            self.replace_market_tape_with_news
        )
        self.replace_market_tape_checkbox.setCursor(Qt.PointingHandCursor)
        self.replace_market_tape_checkbox.setStyleSheet("""
            QCheckBox#ReplaceMarketTapeCheckbox {
                color: #2d2114;
                background: transparent;
                font-size: 12px;
                font-weight: 800;
            ...
            QCheckBox#ReplaceMarketTapeCheckbox::indicator {
                width: 16px;
                height: 16px;
            }

            QCheckBox#ReplaceMarketTapeCheckbox::indicator:unchecked {
                background: rgba(255, 248, 236, 0.78);
                border: 1px solid rgba(83, 59, 33, 0.50);
                border-radius: 3px;
            }

            QCheckBox#ReplaceMarketTapeCheckbox::indicator:checked {
                background: #72542d;
                border: 1px solid #4d351b;
                border-radius: 3px;
                image: url(none);
        ...
        """)
        left_settings_col.addWidget(self.replace_market_tape_checkbox)

        weather_title = QLabel("WEATHER")
        weather_title.setObjectName("SettingsSectionTitle")
        weather_title.setAlignment(Qt.AlignLeft)
        ...
        left_settings_col.addWidget(weather_title)
        left_settings_col.addWidget(self.build_settings_divider())
    ...
    def show_settings_page(self):
        ...
        if hasattr(self, "replace_market_tape_checkbox"):
            self.replace_market_tape_checkbox.setChecked(
                self.replace_market_tape_with_news
            )
    ...
    def apply_news_settings(self):
        ...
        self.hide_reminders = bool(self.hide_reminders_checkbox.isChecked())

        # Applying Settings establishes the newly selected persistent behavior
        # for the active session too.
        self.session_market_tape_hidden = False
        self.session_market_tape_restored = False
    ...
    def open_current_weather_radar_popup(self):
    ...
    def open_weather_radar_popup(
        ...
        location = self.weather_radar_location
        ...
        self.weather_radar_dialog = WeatherRadarDialog(
        ...
        self.weather_radar_dialog.setGeometry(x, y, width, height)
        self.weather_radar_dialog.show()
        self.weather_radar_dialog.raise_()
        self.weather_radar_dialog.activateWindow()
    ...
    def on_weather_loaded(self, rows):
        ...
        if not self.weather_radar_refresh_timer.isActive():
            self.weather_radar_refresh_timer.start()

    def start_weather_radar_preload(self):
        if not self.weather_rows or self.weather_radar_location is None:
            return

        # Every radar-cache pass first warms the current active/final
        # boxscores. This gives the Game Times popups priority over radar
        # network work during launch and scheduled refreshes.
        if not self.radar_preload_waiting_for_boxscores:
            self.radar_preload_waiting_for_boxscores = True

            if not self.sports_games_has_loaded:
                print(
                    "Deferring radar preload until Game Times "
                    "and box-score cache warmup are ready."
                )
                return

            self.start_sports_boxscore_prefetch(
                self.latest_sports_leagues
            )
            return
        ...
        try:
            if (
                self.radar_preload_thread
                and self.radar_preload_thread.isRunning()
            ):
                print("Radar preload is already running.")
                return
        except RuntimeError:
            self.radar_preload_thread = None
            self.radar_preload_worker = None
        ...
        # These are the exact 90-minute windows used by every forecast-hour
        # card in the weather column.
        ...
        self.radar_preload_thread = QThread()
        self.radar_preload_worker = WeatherRadarPreloadWorker(
            latitude=self.weather_radar_location.latitude,
            longitude=self.weather_radar_location.longitude,
        ...
        self.radar_preload_thread.started.connect(
            self.radar_preload_worker.run
        )

        self.radar_preload_worker.finished.connect(
            self.on_weather_radar_preload_finished
        )
        self.radar_preload_worker.failed.connect(
            self.on_weather_radar_preload_failed
        )

        self.radar_preload_worker.finished.connect(
            self.radar_preload_thread.quit
        )
        self.radar_preload_worker.failed.connect(
            self.radar_preload_thread.quit
        )

        self.radar_preload_thread.finished.connect(
            self.radar_preload_worker.deleteLater
        )
        self.radar_preload_thread.finished.connect(
            self.radar_preload_thread.deleteLater
        )
    ...
    def on_weather_radar_preload_finished(
        self,
        ready_count,
        failed_count,
        requested_count,
        ...
        print(
            "Radar preload complete: "
            f"{ready_count}/{requested_count} ready, "
            f"{failed_count} failed."
        )

    def on_weather_radar_preload_failed(self, message):
        print(f"Radar preload failed: {message}")
    ...
    def show_more_news_panel(self):
        if hasattr(self, "bottom_left_stack"):
            self.bottom_left_stack.setCurrentWidget(self.more_news_card)

    def show_market_news_panel(self):
        if hasattr(self, "bottom_right_stack"):
            self.bottom_right_stack.setCurrentWidget(self.market_news_card)

    def hide_reminders_for_current_session(self):
        # Do not save this. The normal reminders panel returns next launch
        # unless the separate Settings checkbox is enabled.
        self.session_reminders_hidden = True
        self.show_more_news_panel()
        print("Reminders hidden for this session only.")

    def show_reminders_for_current_session(self):
        # Header button on the left mini-news card.
        ...
        print("Reminders restored for this session.")

    def start_mini_news_background_refresh(self):
        print("Refreshing both mini-news caches in the background.")
        self.start_more_news_fetch(force=True)
        self.start_market_news_fetch(force=True)

    def start_more_news_fetch(self, force=False):
        if not force and not (
            getattr(self, "calendar_has_no_events", False)
            or getattr(self, "hide_reminders", False)
            or getattr(self, "session_reminders_hidden", False)
        ):
            return

        try:
            if self.more_news_thread and self.more_news_thread.isRunning():
                print("Left mini-news fetch already running; skipping overlap.")
                return
        except RuntimeError:
            self.more_news_thread = None
            self.more_news_worker = None

        self.more_news_request_id += 1
        request_id = self.more_news_request_id
        source_key = self.left_news_source_key

        print(
            f"Starting extra-news fetch request {request_id}: "
            f"source={source_key}"
        )

        self.more_news_card.top_source_label.setText(
            get_news_source_display(source_key)
        )
        # Keep previously cached headlines visible while new data loads.
        # Do not switch the visible dashboard panel during background refresh.
        self.more_news_thread = QThread()
        self.more_news_worker = MoreNewsFetchWorker(
            request_id=request_id,
            source_key=source_key,
        )
        self.more_news_worker.moveToThread(self.more_news_thread)

        self.more_news_thread.started.connect(self.more_news_worker.run)

        self.more_news_worker.finished.connect(self.on_more_news_loaded)
        self.more_news_worker.failed.connect(self.on_more_news_failed)

        self.more_news_worker.finished.connect(self.more_news_thread.quit)
        self.more_news_worker.failed.connect(self.more_news_thread.quit)

        self.more_news_thread.finished.connect(self.more_news_worker.deleteLater)
        self.more_news_thread.finished.connect(self.more_news_thread.deleteLater)

        self.more_news_thread.start()

    def on_more_news_loaded(self, request_id, source_key, articles):
        if request_id != self.more_news_request_id:
            print(f"Ignoring stale extra-news request {request_id}")
            return

        if source_key != self.left_news_source_key:
            print(
                "Ignoring extra-news result because left source changed: "
                f"result={source_key}, current={self.left_news_source_key}"
            )
            return
        ...
        if not articles:
            ...
            self.show_reminders_panel()
            self.reminders.set_calendar_error(
                "No reminders or additional headlines available"
            )
            return
        ...
        print(
            f"Cached {len(articles)} extra headlines "
            f"for {get_news_source_display(source_key)}"
        )

    def on_more_news_failed(self, request_id, error_message):
        if request_id != self.more_news_request_id:
            return

        print(f"Extra-news fetch failed: {error_message}")
    ...
    def market_news_should_be_visible(self):
        return bool(
            getattr(self, "session_market_tape_hidden", False)
            or (
                getattr(self, "replace_market_tape_with_news", False)
                and not getattr(self, "session_market_tape_restored", False)
            )
        )

    def show_market_tape_panel(self):
        if hasattr(self, "bottom_right_stack"):
            self.bottom_right_stack.setCurrentWidget(self.stocks_panel)

    def hide_market_tape_for_current_session(self):
        # Header Hide button only affects the current run of the app.
        self.session_market_tape_hidden = True
        self.session_market_tape_restored = False
        self.show_market_news_panel()
        print("Market Tape hidden for this session only.")

    def show_market_tape_for_current_session(self):
        # Right mini-news header button. Does not modify the saved checkbox.
        self.session_market_tape_hidden = False
        self.session_market_tape_restored = True
        self.show_market_tape_panel()
        self.start_stock_fetch()
        print("Market Tape restored for this session.")

    def clear_market_news_fetch_references(self, thread, worker):
        if self.market_news_thread is thread:
            self.market_news_thread = None

        if self.market_news_worker is worker:
            self.market_news_worker = None

    def start_market_news_fetch(self, force=False):
        if not force and not self.market_news_should_be_visible():
            return

        active_thread = self.market_news_thread

        if active_thread is not None and active_thread.isRunning():
            print("Market Tape replacement fetch already running; ignoring repeat click.")
            return

        self.market_news_request_id += 1
        request_id = self.market_news_request_id
        source_key = self.right_news_source_key

        print(
            f"Starting Market Tape replacement request {request_id}: "
            f"source={source_key}"
        )

        self.market_news_card.top_source_label.setText(
            get_news_source_display(source_key)
        )
        # Keep previously cached headlines visible while new data loads.
        # Do not switch the visible dashboard panel during background refresh.
        thread = QThread()
        worker = MoreNewsFetchWorker(
            request_id=request_id,
            source_key=source_key,
        )

        self.market_news_thread = thread
        self.market_news_worker = worker

        worker.moveToThread(thread)

        thread.started.connect(worker.run)

        worker.finished.connect(self.on_market_news_loaded)
        worker.failed.connect(self.on_market_news_failed)
        ...
        thread.finished.connect(
            lambda current_thread=thread, current_worker=worker:
                self.clear_market_news_fetch_references(
                    current_thread,
                    current_worker,
                )
        )
    ...
    def on_market_news_loaded(self, request_id, source_key, articles):
        if request_id != self.market_news_request_id:
            print(f"Ignoring stale Market Tape replacement request {request_id}")
            return

        if source_key != self.right_news_source_key:
            print(
                "Ignoring Market Tape replacement because right source changed: "
                f"result={source_key}, current={self.right_news_source_key}"
            )
            return
        ...
        self.market_news_card.update_article_list(
            articles,
            page_label="MORE",
        )

        print(
            f"Cached {len(articles)} Market Tape replacement headlines "
            f"for {get_news_source_display(source_key)}"
        )

    def on_market_news_failed(self, request_id, error_message):
        if request_id != self.market_news_request_id:
            return

        print(f"Market Tape replacement failed: {error_message}")
        # Preserve the current panel and any previously cached headlines.
    ...
    def start_stock_fetch(self):
        ...
        thread.finished.connect(
            lambda current_thread=thread, current_worker=worker:
                self.clear_stock_fetch_references(
                    current_thread,
                    current_worker,
                )
        )
    ...
    def start_sports_games_fetch(self):
        if self.sports_games_thread is not None:
            try:
                if self.sports_games_thread.isRunning():
                    print("Game Times refresh already running. Skipping overlap.")
                    return
            except RuntimeError:
                self.sports_games_thread = None
                self.sports_games_worker = None
    ...
    def on_sports_games_loaded(self, leagues):
        ...
        self.latest_sports_leagues = list(leagues or [])
        self.sports_games_has_loaded = True

        # Start the cache warmup immediately, before the visible Game Times
        # panel is updated with clickable active/final games.
        self.start_sports_boxscore_prefetch(
            self.latest_sports_leagues
        )

        if hasattr(self.sports_games, "update_leagues"):
            self.sports_games.update_leagues(
                self.latest_sports_leagues
            )
    ...
    def on_sports_games_failed(self, error_message):
        ...
        if (
            not self.sports_games_has_loaded
            and hasattr(self.sports_games, "set_error")
        ):
            self.sports_games.set_error(
                "Unable to load live MLB, NFL, and NBA games."
            )

    def start_sports_boxscore_prefetch(self, leagues):
        if self.sports_boxscore_prefetch_thread is not None:
            try:
                if self.sports_boxscore_prefetch_thread.isRunning():
                    print("Box-score preload already running. Skipping overlap.")
                    return
            except RuntimeError:
                self.sports_boxscore_prefetch_thread = None
                self.sports_boxscore_prefetch_worker = None
    ...
    def clear_sports_boxscore_prefetch_thread(self):
        ...
        if self.radar_preload_waiting_for_boxscores:
            print(
                "Box-score preload finished. Continuing deferred radar preload."
            )
            QTimer.singleShot(
                0,
                self.start_weather_radar_preload,
